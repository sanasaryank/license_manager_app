...
export default function CustomerTagsPage() {
  ...
  const listOps = useListOperations<CustomerTagListItem>({
    data,
    searchFields: (t) => [resolveTranslation(t.name, lang)],
  ...
  const columns = [
    {
      ...
      sortable: true,
      render: (item: CustomerTagListItem) => resolveTranslation(item.name, lang),
    },
    {
      key: 'itemsCount',
      header: t('customerTags.itemsCount'),
      render: (item: CustomerTagListItem) => item.itemsCount ?? item.items?.length ?? 0,
    },
    {
      key: 'isBlocked',
    ...
    },
    {
      ...
      render: (item: CustomerTagListItem) => item.description || '—',
    },
    {
      key: 'actions',
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-semibold text-gray-900">{t('customerTags.title')}</h1>
